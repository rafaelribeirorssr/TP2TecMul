...
const LEVELS = [
  ...
  },
  {
    ...
    lavaDeath: true,
    platforms: [
      { x: 100,  y: 380, w: 160 }, // plataforma de spawn segura
      { x: 300,  y: 380, w: 100 },
      { x: 520,  y: 320, w:  90 },
      { x: 750,  y: 370, w: 100 },
      { x: 980,  y: 290, w:  90 },
...
this.drawBackground(lvl, W, H)
    ...
    if (lvl.lavaDeath) {
      // Nível 3: chão de lava — visual
      const lavaVisual = this.add.rectangle(W / 2, H - 10, W, 60, 0xff2200)
      lavaVisual.setDepth(1)
      // brilho da lava
      const lavaGlow = this.add.rectangle(W / 2, H - 35, W, 12, 0xff6600)
      lavaGlow.setDepth(2)
      // colisor da lava
      this.lavaGround = this.add.rectangle(W / 2, H - 10, W, 60, 0xff2200)
    ...
    // --- Jogador ---
    const startY = this.currentLevel === 2 ? 330 : 380
    this.player = this.physics.add.sprite(100, startY, 'player')
  ...
  createPlatform(x, y, width) {
    const colors  = [0x5cb85c, 0x7777bb, 0xcc5500]
    const topCol  = [0x6edd6e, 0x9999cc, 0xff7722]
  ...
  createPlayerTexture() {
    ...
    const g = this.make.graphics({ x: 0, y: 0, add: false })
    // Chapéu
    g.fillStyle(0xcc0000); g.fillRect(4, 0, 32, 10)
    g.fillStyle(0xcc0000); g.fillRect(8, 10, 24, 8)
    // Cara
    g.fillStyle(0xffcc99); g.fillRect(8, 18, 24, 16)
    // Olhos
    g.fillStyle(0x000000); g.fillRect(13, 21, 5, 5); g.fillRect(23, 21, 5, 5)
    // Bigode
    g.fillStyle(0x6b3a2a); g.fillRect(10, 29, 20, 4)
    // Macacão
    g.fillStyle(0x1a3ccc); g.fillRect(6, 34, 28, 16)
    // Botões
    g.fillStyle(0xffcc00); g.fillRect(12, 36, 4, 4); g.fillRect(24, 36, 4, 4)
    // Sapatos
    g.fillStyle(0x4a2800); g.fillRect(4, 44, 14, 6); g.fillRect(22, 44, 14, 6)
  ...
  fallInLava() {
    ...
    if (this.lives <= 0) {
      ...
      this.time.delayedCall(600, () => {
        // Nível 3: volta ao nível 1 com 3 vidas
        this.scene.start('GameScene', {
          level: 0,
          score: this.score,
          lives: 3
      ...
      // Respawna na plataforma inicial do nível 3
