...
export class MenuScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.titleText = this.add.text(cx, 70, t.title, {
      fontSize: '48px', fill: '#FFD700', fontStyle: 'bold'
    }).setOrigin(0.5)
 
    // --- Iniciar jogo (começa no nível 1) ---
    this.startBtn = this.add.text(cx, 155, `▶  ${t.start}`, {
    ...
    this.add.text(cx, 235, selectLabel, {
      fontSize: '18px', fill: '#aaaaaa'
    }).setOrigin(0.5)
 
    const xs = [cx - 150, cx, cx + 150]
    for (let i = 0; i < 3; i++) {
      this.makeLevelButton(xs[i], 290, i + 1)
    }
 
    // --- Controlos ---
    this.controlsText = this.add.text(cx, 380, t.controls, {
      fontSize: '14px', fill: '#aaaaaa'
    }).setOrigin(0.5)
 
    // --- Botões de idioma ---
    this.add.text(cx - 40, 445, '🇵🇹 PT', {
      fontSize: '18px', fill: currentLang === 'pt' ? '#FFD700' : '#888888'
    }).setOrigin(0.5).setInteractive({ useHandCursor: true })
      .on('pointerdown', () => { setLang('pt'); this.scene.restart() })
 
    this.add.text(cx + 40, 445, '🇬🇧 EN', {
